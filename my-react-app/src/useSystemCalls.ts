     ...
     */
     const spawn = async () => {
        ...
        state.applyOptimisticUpdate(transactionId, (draft) => {
            if (draft.entities[entityId]?.models?.dojo_starter?.Player) {
                draft.entities[entityId].models.dojo_starter.Player.health =
                    remainingMoves;
            }
        });
        ...
        try {
            ...
            console.log("add plate", await client.actions);
            
            
            // Execute the spawn action from the client
            await client.actions.addPlayer(account);

            // Wait for the entity to be updated with the new state
            await state.waitForEntityChange(entityId, (entity) => {
                return (
                    entity?.models?.dojo_starter?.Player?.health ===
                    remainingMoves
                );
            });
        } catch (error) {
